...
1. **触发条件**：
   ...
   - 外部云函数或定时器可通过 `workflow_dispatch` 触发。

2. **生日检查**：
   - 从 GitHub Secrets 中读取生日数据，而不是直接从文件中读取。
   - 检查当天和接下来几天的生日。
   - 农历生日配置中，`birthday` 的年份字段不会参与计算，仅使用月和日。
...
当前工作流只保留手动触发，避免 `push` 或 `pull_request` 造成重复通知。
...
- **`PROJECT_NAME`**: 邮件发件人显示名，统一使用项目名称。

## 注意事项

- 确保日期格式正确，农历日期需使用正确的农历年份。
- 实际计算农历生日时只使用月和日，年份仅作为配置占位字段。
